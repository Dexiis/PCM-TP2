...
class Blackjack {
    ...
    newDeck() {
        newDeck = [];
        for(let suit = 0; suit < SUIT_NUMBER; suit++) {
            for(let number = 0; number < CARD_NUMBER; number++) {
    ...
    getCardsValue(cards) {
        ...
        for(let i = 0; i < cards.length; i++) {
            if(cards[i].number > 1 && cards[i].number < 11) { // 2-10
                value += cards[i].number;
            } else if(cards[i].number > 10) { // Jack, Queen and King
                value += 10;
            } else if(cards[i].number == 1) { // Aces
                acesNumber++;
            }
        }
        ...
        while(acesNumber > 0 && value > MAX_POINTS) { // Best use of the aces - Prevents busting
            value -= 10;
            acesNumber--;
        }
